...
2. search_query는 사용자의 원래 검색 의도를 보존하면서
   공지 검색에 필요한 핵심 조건을 모두 포함하세요.

   사용자가 직접 언급한 연도, 학기, 시험 종류, 과목명, 교수명,
   분반, 날짜·기간 등의 명시적인 조건은 절대 제거하지 마세요.

   사용자가 명시하지 않은 연도, 학기, 중간고사·기말고사 등의 조건은
   임의로 추측하거나 추가하지 마세요.
