...
- 구체적인 질문의 핵심 키워드 일치율 검증과 관련 공지 최신순 정렬
- `notices.deadline`과 현재 시각을 사용한 신청 가능 공지 필터
...
전체 단위 테스트는 현재 53개입니다.

## 현재 제한

- Supabase `aliases` 테이블과 초기 데이터가 배포되기 전에는 은어 없이 검색합니다.
- `open_notice_search` 필터는 구현됐지만 `notices.deadline` 초기 데이터는 아직 없습니다.
...
- [ ] 실제 공지의 `deadline` 초기 데이터 적재와 수집 과정 연결
...
않습니다. 신청 마감만 `notices.deadline`에 구조화하고, 대회 일정과 시험 정보는 선택된
...
`notices.deadline`을 비교해 조회 시점에 계산합니다.
